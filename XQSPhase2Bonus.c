...
char *botMove(char *spells[], int spellsSize, char *usedWords[], char userWord[], int Tally[], int difficulty, int *indexPtr)
{
    if (difficulty == 0 && *indexPtr == 0) // difficulty --> easy and bot is starting the game
    {
        for (int i = 0; i < spellsSize; i++)
        {
            char *userHelpingWord = finishGame(spells, spellsSize, usedWords, spells[i], false, Tally);
            if (userHelpingWord != NULL)
                return spells[i];
        }
        return spells[0];
    }

    if (difficulty == 1 && *indexPtr == 0) // difficulty --> medium and bot is starting the game
    {
        for (int i = 0; i < spellsSize; i++)
        {
            size_t len = strlen(spells[i]);
            char lastCharacter = spells[i][len - 1];
            if (Tally[lastCharacter - 'a'] != 0)
                return spells[i];
        }
        return spells[0];
    }

    if (difficulty == 2 || (difficulty == 1 && (*indexPtr + 1) % 3 == 0)) // difficulty is hard or medium but on an iteration being a multiple of 3
    {
    ...
    int indexOfgoodAvailableBotWords = 0;
    ...
    for (int i = 0; i < spellsSize; i++)
    {
        ...
        {
            goodAvailableBotWords[indexOfgoodAvailableBotWords++] = spells[i];
        }
    }
    ...
    for (int i = 0; i < indexOfgoodAvailableBotWords; i++)
    {
        ...
        for (int j = 0; j < spellsSize; j++)
        {
            ...
            if (charMatching(goodAvailableBotWords[i], spells[j]) && !spellAlreadyUsed(usedWords, spellsSize, spells[j]))
            {
                ...
                if (Tally[lastCharacter - 'a'] == 0 && badWord == true) // only add the bad to the array badAvailableBotWords once
                {
        ...
        bool Asess = wordAsess(badAvailableBotWords, goodAvailableBotWords[i], indexOfbadAvailableBotWords); // true if a word is in the list of bad available words

        probaCounter = 0;
        if (Asess == false && ((difficulty == 2 || (difficulty == 1 && (*indexPtr + 1) % 3 == 0)) || (badAvailableBotWords[0] == NULL && (difficulty == 0 || difficulty == 1))))
        {
    ...
    if (difficulty == 2 || (difficulty == 1 && badAvailableBotWords[0] == NULL) || (difficulty == 1 && (*indexPtr + 1) % 3 == 0))
    {
        for (int i = 0; i < indexOfgoodAvailableBotWords; i++)
        {
            if (proba[i] > max)
    ...
    else if (difficulty == 0 && badAvailableBotWords[0] == NULL)
    {
        double min = 1;
        for (int i = 0; i < indexOfgoodAvailableBotWords; i++)
        {
            size_t len = strlen(goodAvailableBotWords[i]);
            char lastLetter = goodAvailableBotWords[i][len - 1];
            if (Tally[lastLetter - 'a'] != 0 && proba[i] < min)
            {
                min = proba[i];
                winningWordIndex = i;
            }
        }
        if (min == 1) // if all the available words are finishing words we choose the first one
        {
            for (int i = 0; i < indexOfgoodAvailableBotWords; i++)
            {
                if (proba[i] < min)
...
int GamePlay(int *indexPtr, char *usedWords[], int Tally[], bool player1Turn, char player1[], char player2[], char *spells[], int difficulty)
{
    ...
    while (gameInProgress)
    {
        ...
        else
        {
            ...
            {
                usedWords[*indexPtr] = botMove(spells, 78, usedWords, usedWords[*indexPtr - 1], Tally, difficulty, indexPtr);
                printf("%s will pick a spell from the following List: %s\n", player2, usedWords[*indexPtr]);
                player1Turn = true;
                usedWords[77] = "";
                usedWords[76] = "";
...
int main()
{
    ...
    scanf(" %s", player1);
    printf("Welcome %s!\n", player1);
    char player2[100] = "Bot";

    // Step 3
    PrintingList(spells, 78);
    int difficulty = -1;
    printf("What difficulty would like to play with (Easy (0) , Medium (1) , Hard (2) ): ");
    scanf("%d", &difficulty);
    ...
    player1Turn = GamePlay(&index, usedWords, Tally, player1Turn, player1, player2, spells, difficulty);
